...
// TODO: Waiting for things to finish shutting down.
...
impl Daemon {
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            tokio::spawn(self.process_socket(socket.0));
        }
    }
        ...
        tokio::select! {
            _ = async {
                loop {
                    ...
                    match self::Daemon::process_data(n, &buf[0..n], socket_addr.to_string().as_str()) {
                        None => debug!("lll"),
